class ETAInvoiceExporter {
  ...
  async exportCurrentPage(format, options) {
    ...
    // Extract addresses if address fields are selected
    const shouldExtractAddresses = (options.sellerAddress && options.sellerAddress !== undefined) || 
                                  (options.buyerAddress && options.buyerAddress !== undefined);
    
    if (shouldExtractAddresses) {
      this.showStatus('جاري استخراج عناوين الفواتير...', 'loading');
      dataToExport = await this.loadInvoiceAddresses(dataToExport, tab.id, options);
    }
  ...
  createMainSheetWithExactLayout(wb, data, options) {
    ...
    data.forEach((invoice, index) => {
      const row = [
        ...
        this.getAddressValue(invoice.sellerAddress),         // Q - عنوان البائع
        invoice.buyerTaxNumber || '',                        // R - الرقم الضريبي للمشتري
        invoice.buyerName || '',                             // S - اسم المشتري
        this.getAddressValue(invoice.buyerAddress),          // T - عنوان المشتري
    ...
    // Add automatic filter to all columns
    this.addAutoFilter(ws, headers.length);
  ...
  createDetailsSheetsWithLinks(wb, data, options) {
    data.forEach((invoice, index) => {
      ...
      const detailRows = [
        ...
        ['عنوان البائع:', this.getAddressValue(invoice.sellerAddress), '', '', '', '', '', '', '', ''],
        ['المشتري:', invoice.buyerName || '', '', '', '', '', '', '', '', ''],
        ['عنوان المشتري:', this.getAddressValue(invoice.buyerAddress), '', '', '', '', '', '', '', ''],
        ['', '', '', '', '', '', '', '', '', ''],
        detailHeaders
      ];
      ...
      // Add auto filter to details sheet (only for the data rows, not the header info)
      this.addAutoFilterToDetailsSheet(ws, detailHeaders.length);
  ...
  getAddressValue(address) {
    // Ensure seller and buyer address columns always have a value
    // If address is missing, empty, or contains only whitespace, use 'غير محدد'
    // This ensures consistent data structure and prevents empty cells in Excel
  ...
  addAutoFilter(ws, columnCount) {
    // Add auto filter to the header row (row 0) for all columns
    // This enables Excel's built-in filter functionality where users can:
    // - Sort data by clicking on column headers
    // - Filter data using dropdown menus on each column
    ...
    // Set the auto filter property on the worksheet
    // This tells Excel to apply filters to the specified range
    ws['!autofilter'] = {
      ref: filterRange
    };
    
    // Add filter icons to header cells by updating their style
    // This provides visual indication that filters are available
    const range = XLSX.utils.decode_range(ws['!ref']);
    for (let col = 0; col < columnCount; col++) {
      const cellAddress = XLSX.utils.encode_cell({ r: 0, c: col });
      if (ws[cellAddress] && ws[cellAddress].s) {
        // Add a subtle indicator that this column has a filter
        // The actual filter dropdown will be handled by Excel
  ...
  addAutoFilterToDetailsSheet(ws, columnCount) {
    // For details sheets, the header row is at row 9 (after invoice info including addresses)
    // The filter range is from A10 to the last column in row 10
    // This enables filtering on the invoice line items (products/services)
    
    const lastColumn = XLSX.utils.encode_col(columnCount - 1);
    const filterRange = `A10:${lastColumn}10`;
    
    // Set the auto filter property on the worksheet
    // This enables filtering on the product/service details
    ws['!autofilter'] = {
      ref: filterRange
    };
  ...
  formatDetailsWorksheet(ws, headers) {
    ...
    const range = XLSX.utils.decode_range(ws['!ref']);
    const headerRow = 9; // Updated to account for additional address rows
    ...
    for (let row = 10; row <= range.e.r - 2; row++) { // Updated to start from row 10
      const isEvenRow = (row - 10) % 2 === 0; // Updated calculation
      const fillColor = isEvenRow ? "F8F9FA" : "FFFFFF";
